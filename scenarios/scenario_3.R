# Scenario 3 - ramp-up accrual of participants ----
...
sims <- 10000
cores <- 15
stage_n <- seq(200, 3000, 200)
p1tru <- rep(0.1, 6)
p2tru <- seq(0.1, 0.05, -0.01)
resp_delay <- function(n) runif(n, 48L, 72L)
enro_rate <- 25
enro_intensity = function(t)
  (t < 52)*2/52 + (t>=52 & t<78)*10/52 + (t>=78 & t<104)*20/52 + (t>=104 & t<156)*45/52 + (t>=156)
...
saveRDS(dat, "~/out_files/optimum_sims/scenario_3_dat.rds")
saveRDS(dec, "~/out_files/optimum_sims/scenario_3_dec.rds")
